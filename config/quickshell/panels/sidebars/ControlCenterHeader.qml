...
Section {
    ...
    readonly property var _menuActions: [
        ...
        },
        {
            ...
            label: "SUSPEND",
            act: "cmd",
            cmd: "hyprctl clients -j | jq -r '.[].pid' | xargs kill 2>/dev/null; systemctl suspend",
            inShell: true,
            group: 1
        },
        {
            ...
            label: "HIBERNATE",
            act: "cmd",
            cmd: "hyprctl clients -j | jq -r '.[].pid' | xargs kill 2>/dev/null; systemctl hibernate",
            inShell: true,
            group: 1
        },
        {
            ...
            label: "LOG OUT",
            act: "cmd",
            cmd: "hyprctl clients -j | jq -r '.[].pid' | xargs kill 2>/dev/null; pkill Hyprland || loginctl terminate-user $USER",
            inShell: true,
            group: 2
        },
        {
            ...
            label: "REBOOT",
            act: "cmd",
            cmd: "hyprctl clients -j | jq -r '.[].pid' | xargs kill 2>/dev/null; systemctl reboot",
            inShell: true,
            danger: true,
            group: 2
        },
        {
            ...
            label: "SHUT DOWN",
            act: "cmd",
            cmd: "hyprctl clients -j | jq -r '.[].pid' | xargs kill 2>/dev/null; systemctl poweroff",
            inShell: true,
            danger: true,
            group: 2
        }
    ...
    function _runAction(a: var): void {
        ...
        case "settings":
            ...
            return;
        case "reload":
            ProcessPool.runDetached("hyprctl reload && pkill -TERM -x qs && sleep 1 && nohup qs >/dev/null 2>&1 &", {
                shell: true
            });
            return;
        case "lock":
    ...
    ColumnLayout {
        ...
        Rectangle {
            ...
            antialiasing: true
            color: idMa.containsMouse || header.menuOpen ? Theme.controlBackgroundHover : "transparent"
