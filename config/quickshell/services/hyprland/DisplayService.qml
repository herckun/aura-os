...
Singleton {
  ...
  function updateMonitor(output: string, mode: string, scale: string, position: string): void {
    ...
    if (!found) {
      copy.push({ output: output, mode: mode, scale: scale, position: position })
    }
    configEntries = copy
    hasPending = true
  }
  ...
  function resetToDefaults(): void {
    _snapshotIfNeeded()
    configEntries = [{ output: "", mode: "preferred", position: "auto", scale: "1.0" }]
    hasPending = true
  }

  function applyPending(): void {
    if (!_previousConfig) return
  ...
  function revert(): void {
    ...
    countdownRemaining = 0
    if (_previousConfig) {
      configEntries = _previousConfig.map(function(e) { return Object.assign({}, e) })
      var content = _buildConfigContent(configEntries)
      _writeConfig(content, function() {
        _previousConfig = null
        _applyContent = ""
  ...
  function getConfigForOutput(output: string): var {
    for (var i = 0; i < configEntries.length; i++) {
      if (configEntries[i].output === output) return configEntries[i]
    }
    return null
  }
  ...
  function _findMatchingMode(monitor: var): string {
    ...
    for (var i = 0; i < monitor.availableModes.length; i++) {
      ...
      if (parsed && parsed.width === w && parsed.height === h &&
          (rr === 0 || Math.abs(parsed.refreshRate - rr) < 1)) {
  ...
  function _parseModeString(modeStr: string): var {
    ...
    var label = w + "x" + h
    if (!isNaN(rr) && rr > 0) label += "@" + Math.round(rr) + "Hz"
  ...
  function _snapshotIfNeeded(): void {
    if (!_previousConfig && hasPending === false) {
      _previousConfig = configEntries.map(function(e) { return Object.assign({}, e) })
    }
  }
  ...
  function _syncConfigFromMonitors(): void {
    ...
    for (var k = 0; k < existing.length; k++) {
      ...
      for (var l = 0; l < merged.length; l++) {
        if (merged[l].output === existing[k].output) { found = true; break }
      }
      if (!found) merged.push(existing[k])
    }
  ...
  function _buildConfigContent(entries: var): string {
    ...
    for (var j = 0; j < entries.length; j++) {
      if (entries[j].output === "") { hasFallback = true; break }
    }
    if (!hasFallback) {
