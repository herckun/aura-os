...
BasePlugin {
  ...
  manifest: ({
    ...
    settings: [
      ...
      { key: "showBackground", label: "BACKGROUND", description: "Show background behind widget", type: "toggle", default: true },
      { key: "fontSize", label: "FONT SIZE", description: "Lyrics text size", type: "stepper", min: 16, max: 32, step: 2, unit: "px", default: 22 },
      { key: "maxLines", label: "VISIBLE LINES", description: "Number of lyric lines to show", type: "stepper", min: 3, max: 11, step: 2, default: 7 },
      { key: "widgetWidth", label: "WIDTH", description: "Widget width", type: "stepper", min: 300, max: 600, step: 20, unit: "px", default: 420 }
    ]
  })
  ...
  property Component desktopComponent: Item {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            SequentialAnimation on height {
              ...
              NumberAnimation { from: 4; to: 16; duration: 400 + index * 100; easing.type: Easing.InOutQuad }
              NumberAnimation { from: 16; to: 4; duration: 400 + index * 100; easing.type: Easing.InOutQuad }
            }
          }
        }
      }
    ...
    Item {
      ...
      visible: !LyricsService.hasLyrics || MediaService.playbackStatus === "Stopped" ||
               (LyricsService.hasLyrics && LyricsService.currentLineIndex < 0 && MediaService.playbackStatus === "Playing") ||
               (LyricsService.hasLyrics && LyricsService.currentLineIndex >= LyricsService.lines.length - 1 && LyricsService.lines.length > 0)
      ...
      Column {
        ...
        Icon {
          source: {
            if (LyricsService.loading) return Icons.get("search")
            if (!MediaService.hasPlayer) return Icons.get("music-note")
            if (MediaService.playbackStatus === "Stopped") return Icons.get("stop")
            return ""
          }
          size: 48
          color: lyricsContainer._dimColor
        ...
        Text {
          text: {
            if (LyricsService.loading) return "Looking for lyrics..."
            if (!MediaService.hasPlayer) return "Play something"
            if (MediaService.playbackStatus === "Stopped") return "Playback stopped"
            if (!LyricsService.hasLyrics) return "No lyrics found"
            if (LyricsService.currentLineIndex < 0) return "Get ready"
    ...
    Item {
      ...
      visible: LyricsService.hasLyrics &&
               ...
               LyricsService.currentLineIndex >= 0 &&
               LyricsService.currentLineIndex < LyricsService.lines.length - 1
      clip: true
      ...
      Rectangle {
        ...
        z: 2
        gradient: Gradient {
          GradientStop { position: 0.0; color: lyricsContainer._bgColor }
      ...
      Rectangle {
        ...
        gradient: Gradient {
          ...
          GradientStop { position: 1.0; color: lyricsContainer._bgColor }
        }
      }
      ...
      Column {
        ...
        Behavior on y { enabled: Theme.animationsEnabled; NumberAnimation { duration: Theme.animationNormal; easing.type: Easing.OutQuad } }

        Repeater {
          model: {
            if (!LyricsService.hasLyrics || LyricsService.lines.length === 0) return []
            var result = []
          ...
          delegate: Item {
            ...
            Text {
              ...
              text: modelData.text
              color: {
                if (modelData.isCurrent) return lyricsContainer._accentColor
                if (modelData.distance <= 1) return lyricsContainer._textColor
                return lyricsContainer._dimColor
              }
              font.pixelSize: {
                if (modelData.isCurrent) return lyricsContainer._fontSize
                if (modelData.distance <= 1) return lyricsContainer._fontSize - 2
                return lyricsContainer._fontSize - 4
              }
              font.weight: modelData.isCurrent ? Font.Bold : (modelData.distance <= 1 ? Font.Medium : Font.Normal)
              font.family: Theme.fontFamilyDisplay
              opacity: {
                if (LyricsService.currentLineIndex < 0) return 0.6
                if (modelData.isCurrent) return 1.0
                if (modelData.distance === 1) return 0.6
                if (modelData.distance === 2) return 0.35
                return 0.15
              }
    ...
    Item {
      ...
      Row {
        ...
        Text {
          text: {
            var idx = LyricsService.currentLineIndex
            if (idx < 0) return "— / " + LyricsService.lines.length
