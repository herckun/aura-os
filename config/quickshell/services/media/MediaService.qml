...
Singleton {
  ...
  function playPause(): void { activePlayer?.togglePlaying() }
  ...
  function setActivePlayer(player: MprisPlayer): void {
    activePlayer = player
    if (player) _persistIdentity(player.identity)
  }
  ...
  readonly property list<MprisPlayer> availablePlayers: {
    ...
    if (excluded.length === 0) return players.filter(_isRealPlayer)
    return players.filter(function(p) {
      if (!_isRealPlayer(p)) return false
      ...
      return !excluded.some(function(ex) {
        ...
        if (identity.includes(exLower) || desktopEntry.includes(exLower)) return true
        if (exLower.indexOf(".") !== -1) {
          var lastPart = exLower.split(".").pop()
          if (lastPart && (identity.includes(lastPart) || desktopEntry.includes(lastPart))) return true
        }
        if (identity.length >= 3 && exLower.includes(identity)) return true
        return false
      })
    })
  }
  ...
  function _isRealPlayer(player): bool {
    ...
    if (isFirefoxYoutubeHoverPreview(player)) return false
  ...
  function isFirefoxYoutubeHoverPreview(player: MprisPlayer): bool {
    ...
    if (!id.includes("firefox")) return false
    var url = ""
    try { url = (player.metadata?.["xesam:url"] || "").toString() } catch (e) {}
  ...
  function _resolveActivePlayer(): void {
    var playing = availablePlayers.find(function(p) { return p.isPlaying })
    if (playing) {
    ...
    if (savedId) {
      var match = availablePlayers.find(function(p) { return p.identity === savedId })
      if (match && !isIdle(match)) {
        activePlayer = match
        return
      }
    }
    activePlayer = availablePlayers.find(function(p) { return p.canControl && !isIdle(p) }) ?? null
    if (activePlayer) _persistIdentity(activePlayer.identity)
  }

  function _persistIdentity(identity: string): void {
    if (identity) Store.set("media.lastPlayerIdentity", identity)
  }
  ...
  function _syncFromActive(): void {
    ...
    var trackChanged = newTitle !== svc.currentTitle ||
                       newArtist !== svc.currentArtist ||
                       newAlbum !== svc.currentAlbum
  ...
  function _startAudioAnalyzer(): void {
    if (svc._eqHandle) return

    var confPath = AppInfo.configHome + "/features/system/cava-config.conf"
    svc._eqHandle = WatchService.register("media-eq",
      ["cava", "-p", confPath],
      function(line) { svc.eqBands = _parseCavaLine(line) ?? [0, 0, 0, 0, 0, 0, 0, 0] },
      function() { svc._eqHandle = null }
    )
  }
  ...
  function _stopAudioAnalyzer(): void {
    svc._eqHandle?.stop()
    svc._eqHandle = null
    if (svc.eqBands[0] !== 0) svc.eqBands = [0, 0, 0, 0, 0, 0, 0, 0]
  }
  ...
  Connections {
    ...
    function onTrackChanged() { _syncFromActive() }
    function onPostTrackChanged() { _syncFromActive() }
    ...
    function onTrackAlbumChanged() { _syncFromActive() }
    function onTrackArtUrlChanged() { _syncFromActive() }
  ...
  Component.onCompleted: {
    ...
    if (svc.activePlayer) _syncFromActive()
  }
