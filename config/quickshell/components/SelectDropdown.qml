...
Rectangle {
  ...
  Popup {
    ...
    contentItem: ListView {
      ...
      delegate: Rectangle {
        ...
        color: {
          if (itemMouse.containsMouse) return Theme.controlBackgroundHover
          var itemValue = modelData[root.valueRole] || modelData.value || modelData
          if (itemValue === root.value) return Qt.rgba(Theme.accent.r, Theme.accent.g, Theme.accent.b, 0.1)
          return "transparent"
        }
  ...
  function _updateDisplayText(): void {
    if (!root.value || !root.items || root.items.length === 0) return
